...
val jdbcdf=spark.read.format("jdbc").option("url","jdbc:postgresql://localhost:5432/postgres").option("driver","org.postgresql.Driver").option("dbtable","public.products").option("user","postgres").option("password","1").load()
val ordersum=order.groupBy($"man_code" as "manu_code",$"prod_code").count()
val mergedf=ordersum.join(jdbcdf,jdbcdf("pro_code").contains(ordersum("prod_code")),"inner")
...
val del2=del1.groupBy($"man_code" as "manu_code",$"prod_code").agg(count("_ordernumber") as "quantity",first("add_street") as "street",first("add_zipcode") as "zipcode",first("city_id") as "cityid",first("cus_id") as "cusid",first("cus_lastname") as "lustname",first("cus_name") as "name",first("_ordernumber") as "ordernumber")
val del3=del2.join(jdbcdf,jdbcdf("pro_code").contains(del2("prod_code")),"inner")
